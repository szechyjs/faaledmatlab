...
for test_index = 1:length(test_values_rand)
    ...
    data = ([0*ones(1,1) test_values_rand(test_index)*ones(1,13) 0*ones(1,1)])';
...
%store and save data%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%
subject_data_26 = struct('SubjectNumber', sub_num, 'ControlVoltageOutput', voltages_tested, 'ApproximateI', intensities_tested, 'ResponseMatrix', responses, 'YesResponseTime', res_time_yes, 'NoResponseTime', res_time_no);

filename = sprintf('%s %d %s', 'Subject', sub_num, 'P26.mat');
save(filename, 'subject_data_26')
