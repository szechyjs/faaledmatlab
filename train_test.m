function train_test(pulse_length, delay_length, sub_num)
    ...
    if(~exist('delay_length','var'))
        delay_length = input('What is the delay length (ms)?    ');
    end
    ...
    for test_index = 1:length(test_values_rand)
        ...
        data = ([0*ones(1,1) test_values_rand(test_index)*ones(1,pulse_length) 0*ones(1,delay_length) test_values_rand(test_index)*ones(1,pulse_length) 0*ones(1,1)])';
